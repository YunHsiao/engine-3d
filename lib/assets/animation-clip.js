...
export default class AnimationClip extends Asset {
  ...
  sampleAndBlend(skeleton, t, weight) {
    clamp(t, 0, this._length);

    let blendPos = (joint, pos) => vec3.lerp(joint.lpos, joint.lpos, pos, weight);
    let blendScale = (joint, scale) => vec3.lerp(joint.lscale, joint.lscale, scale, weight);
    let blendRot = (joint, rot) => quat.slerp(joint.lrot, joint.lrot, rot, weight);
    ...
    for (let i = 0; i < this._framesList.length; ++i) {
      ...
      if (idx == 0)
      {
        for (let j = 0; j < frames.joints.length; ++j) {
          ...
          if (jointFrames.translations) {
            blendPos(joint, jointFrames.translations[0]);
          }

          if (jointFrames.rotations) {
            blendRot(joint, jointFrames.rotations[0]);
          }

          if (jointFrames.scales) {
            blendScale(joint, jointFrames.scale[0]);
          }
        }
      }
      else
