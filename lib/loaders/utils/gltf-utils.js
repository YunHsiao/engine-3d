...
function createMesh(app, gltf, bin, index) {
  ...
  if (attributes.POSITION !== undefined) {
    ...
    meshAsset._minPos = vec3.create(acc.min[0], acc.min[1], acc.min[2]);
    meshAsset._maxPos = vec3.create(acc.max[0], acc.max[1], acc.max[2]);
  }
  ...
  let compType = accessors[attributes.POSITION].componentType;
  let vbData = new _compType2Array[compType](bin, vbView.byteOffset, 
    vbView.byteLength / gfx.attrTypeBytes(compType));
  ...
  for (let i = 0; i < gltfMesh.primitives.length; ++i) {
    ...
    if (primitive.indices !== undefined) {
      ...
      let ibData = new _compType2Array[ibAcc.componentType](bin, ibView.byteOffset,
