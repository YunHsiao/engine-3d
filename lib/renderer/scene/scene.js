...
export default class Scene {
  ...
  /**
   * update built-in bounding shapes for all models,
   * used in the frustum culling process
   */
  tick() {
    for (let i = 0; i < this._models.length; ++i) {
      let model = this._models.data[i];
      /* here both condition is necessary:
        `node._dirty` catches the case when there is no `transformChanged` event
        emitted on this node, e.g. model parent's transform changed
        `model._dirty` catches every other case when
        there is a `transformChanged` event. */
      // TODO: but what about when user invoked update right after model parent changed?
      if (model._node._dirty || model._dirty) {
