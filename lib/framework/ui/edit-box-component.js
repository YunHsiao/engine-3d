...
import { color4, vec3, quat } from '../../vmath';
...
export default class EditBoxComponent extends  UIElementComponent{
  ...
  _updateMatrix() {
    let widget = this._textComp.entity.getComp('Widget');
    let corner = [vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0)];
    widget.getWorldCorners(corner[0], corner[1], corner[2], corner[3]);
    let rot = this._entity.getWorldRot(quat.create());
    let angle = -quat.getAxisAngle(vec3.create(0, 0, 1), rot);
    this._activeDom.style.transformOrigin = 'bottom left';
    let origin = { a: Math.cos(angle), b: Math.sin(angle), c: -Math.sin(angle), d: Math.cos(angle), tx: corner[1].x, ty: -corner[1].y };
    // TODO: add scale
    let matrix = `matrix(${origin.a}, ${origin.b}, ${origin.c}, ${origin.d}, ${origin.tx}, ${origin.ty})`;
    this._activeDom.style.transform = matrix;
  }
  ...
  _onBlur() {
    ...
    this._highlighting = false;
    this._updateState();
    if (this._activeDom && this._activeDom === document.activeElement) {
      this._activeDom.blur();
    }
  }
