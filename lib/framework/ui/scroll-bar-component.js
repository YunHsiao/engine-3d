...
export default class ScrollBarComponent extends UIElementComponent {
  constructor() {
    ...
    this._onMouseDown = e => {
      ...
      if (e.button === 'left') {
        ...
        if (e.target !== this._handle) {
          if (!this._dragging) {
            this._dragging = true;
            this._updateValue(vec2.create(e.mouseX, e.mouseY), true);
          }
        } else {
          this._dragging = true;
          this._startDistance = this._startOffset(vec2.create(e.mouseX, e.mouseY));
        }
      }
    };
    ...
    this._onMouseMove = e => {
      ...
      if (e.button === 0) {
        ...
        if (this._dragging) {
          this._updateValue(vec3.create(e.mouseX, e.mouseY, 0));
        }
      }
    };
    ...
    this._onTouchStart = e => {
      ...
      this._pressing = true;
      this._updateState();
      this._startPos = vec3.create(e.x, e.y, 0);
      this._offsetValue = 0.0;
      if (e.target !== this._handle) {
        if (!this._dragging) {
          this._dragging = true;
          this._clkBar(vec3.create(e.x, e.y, 0), true);
        }
      } else {
        this._dragging = true;
        this._startDistance = this._startOffset(vec2.create(e.x, e.y));
      }
    };
    ...
    this._onTouchMove = e => {
      ...
      if (e.id === this._fingerId) {
        if (this._dragging) {
          this._updateValue(vec3.create(e.x, e.y, 0));
        }
      }
    };
  ...
  /**
   * @param {number} val - sets the value of the current value
   */
  _set(val) {
  ...
  _updateValue(mouse, isClk = false) {
    ...
    let position = vec2.create(corners[1].x, corners[1].y);
    let calVec = vec2.create(0, 0),
      mouseVec = vec2.create(0, 0);
    let calPos = this._direction === 'horizontal' ? vec2.create(corners[2].x, corners[2].y) : vec2.create(corners[0].x, corners[0].y);
   ...
   */
  _startOffset(mouse) {
    let parentCorners = [vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0)];
    let calWidget = this._getCalculateWidget();
    calWidget.getWorldCorners(parentCorners[0], parentCorners[1], parentCorners[2], parentCorners[3]);
    let calVec = vec3.create(0, 0, 0);
    if (this._direction === 'horizontal') {
    ...
    // get handle center
    let center = vec3.create(0, 0, 0), delta = vec2.create(0, 0);
    vec3.add(center, corners[3], corners[1]);
    vec3.divide(center, center, vec3.create(2, 2, 2));
    // get distance from mouse to center
    vec2.subtract(delta, mouse, vec2.create(center.x, center.y));
    let distance = vec2.dot(delta, vec2.create(calVec.x, calVec.y)) / (this._direction === 'horizontal' ? calWidget._rect.w : calWidget._rect.h);
    return distance;
  }
