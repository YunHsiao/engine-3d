...
export default class Bounds {
  ...
  constructor(center, size) {
    this._center = vec3.create(0, 0, 0);
    vec3.copy(this._center, center);
    this._extents = vec3.create(0, 0, 0);
    vec3.mul(this._extents,size, vec3.create(0.5, 0.5, 0.5));
    this._size = vec3.create(0, 0, 0);
    this._min = vec3.create(0, 0, 0);
    this._max = vec3.create(0, 0, 0);
  }
   ...
   */
  set size(val) {
    vec3.mul(this._extents, val, vec3.create(0.5, 0.5, 0.5));
  }
   ...
   */
  get size() {
    let value = vec3.create(0, 0, 0);
    vec3.mul(value, this._extents, vec3.create(2, 2, 2));
    return value;
  }
  ...
  setMinMax(min, max) {
    ...
    vec3.mul(value, value, vec3.create(0.5, 0.5, 0.5));
   ...
   */
  encapsulate(point) {
    let min = vec3.create(0, 0, 0), max = vec3.create(0, 0, 0);
