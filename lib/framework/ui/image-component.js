...
function _reallocVertexData(type, fillType) {
  ...
  for (let i = 0; i < vertexCount; ++i) {
    wposList[i] = vec3.create(0, 0, 0);
    lposList[i] = vec3.create(0, 0, 0);
    uvs[i] = vec3.create(0, 0, 0);
  }
...
const _genFilledRadialVerts = (() => {
  ...
  let intersectionPoints1 = [vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0)];
  let intersectionPoints2 = [vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0)];
  let intersectionUVs1 = [vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0)];
  let intersectionUVs2 = [vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0)];

  let center = vec3.create(0, 0, 0);
  let centerUVs = vec3.create(0, 0, 0);
  // quad vert data, in counter clockwise order
  let quadVertPoses = [vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0), vec3.create(0, 0, 0)];
  let quadVertAngles = [0, 0, 0, 0];
  let uvXaxis = vec3.create(0, 0, 0);
  let uvYaxis = vec3.create(0, 0, 0);
  ...
  let rawQuad = {
    ...
    bottom: 0,
    top: 100,
    uvbl: vec3.create(0, 0, 0),
    uvbr: vec3.create(1, 0, 0),
    uvtl: vec3.create(0, 1, 0),
    uvtr: vec3.create(1, 1, 0)
  };
...
export default class ImageComponent extends UIElementComponent {
  onInit() {
    ...
    this._entity.on('Widget.onRectChanged', () => {
      this._onRectChanged();
    });
  }

  onDestroy() {
    this._entity.off('Widget.onRectChanged', () => {
      this._onRectChanged();
    });
  }
