...
export default class GridLayoutComponent extends UIElementComponent {
  constructor() {
     ...
     * @type {vec2}
     */
    this._lastSize = vec2.create(0, 0);
  ...
  onInit() {
    ...
    this._lastSize = vec2.create(this._widget._rect.w, this._widget._rect.h);
  }

  tick() {
    if (!vec2.equals(this._lastSize, vec2.create(this._widget._rect.w, this._widget._rect.h))) {
      this._lastSize = vec2.create(this._widget._rect.w, this._widget._rect.h);
      this._dirty = true;
    }
  ...
  _calculate() {
    ...
    let contentSize = vec2.create(
      this._cellWidth * actualX + this._spacingX * (actualX - 1),
      this._cellHeight * actualY + this._spacingY * (actualY - 1)
    );
    let starOffset = vec2.create(this.getStartOffset('horizontal', contentSize.x), this.getStartOffset('vertical', contentSize.y));
