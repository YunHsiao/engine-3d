...
export default class SliderComponent extends UIElementComponent {
  constructor() {
    ...
    /**
     * Normalized value, ignore the reverse, rang[0,1]
     * @type {number}
     */
    ...
    this._onMouseDown = e => {
      ...
      if (e.button === 'left') {
        ...
        if (!this._handle || e.target !== this._handle) {
          this._updateDrag(vec3.create(e.mouseX, e.mouseY, 0));
        }
    ...
    this._onTouchMove = e => {
      ...
      if (e.id === this._fingerId) {
        if (this._dragging) {
          this._updateDrag(vec3.create(e.x, e.y, 0));
        }
      }
    };
  ...
  _updateDrag(mousePos) {
    ...
    if (containerWidget) {
      ...
      let containerVec = vec3.create(0, 0, 0), offsetVec = vec3.create(0, 0, 0);
  ...
  /**
   * Calculation of normalization due to external modification
   * @param {number} val
   */
  _clamp01NormalizeValue(val) {
    let value = val / (this._calMaxValue - this._calMinValue);
    this._normalizedValue = this._reverse ? 1 - value : value;
  }
...
SliderComponent.schema = {
  ...
  reverse: {
    ...
    set(val) {
      ...
      this._reverse = val;
      this._clamp01NormalizeValue(this._value);
      this._updateVisuals();
    }
  },
  ...
  value: {
    ...
    set(val) {
      ...
      this._clamp01NormalizeValue(val);
      this._set(val);
    }
  },
