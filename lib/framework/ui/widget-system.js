...
export default class WidgetSystem extends System {
  ...
  tick() {
    ...
    for (let index = 0; index < this._screens.length; ++index) {
      ...
      utils.walkSoftBreak(screen._entity, ent => {
        if (ent.active === true) {
          _entities.push(ent);
          return true;
        } else {
  ...
  _processLayout(entities) {
    for (let i = 0; i < entities.length; ++i) {
      ...
      if (parentWidget === null) {
        // TODO:
        let screen = entity.getComp('Screen');
        let scaleFactor = screen ? screen.scaleFactor : 1;
        // width = screen.width;
        ...
        parentWidth = this._app._canvas.width / scaleFactor;
        parentHeight = this._app._canvas.height / scaleFactor;
