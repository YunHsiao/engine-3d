...
export default class VelocityOvertimeModule {
  ...
  animate(p) {
    ...
    let vel = vec3.create(this._x.evaluate(normalizedTime, p.randomSeed), this._y.evaluate(normalizedTime, p.randomSeed), this._z.evaluate(normalizedTime, p.randomSeed));
