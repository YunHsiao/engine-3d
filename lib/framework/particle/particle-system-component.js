...
export default class ParticleSystemComponent extends Component {
  ...
  emit(count, emitParams = null) {
    ...
    for (let i = 0; i < count; ++i) {
      let particle = this._renderer._getFreeParticle();
      if (particle === null)
        return;
      ...
      this._renderer._setNewParticle(particle);
  ...
  _updateParticles(dt) {
    ...
    for (let i = 0; i < this._particles.length; ++i) {
      ...
      if (p.remainingLifetime < 0.0) {
        ...
        //       subEmitter.emitter.entity.setLocalPos(p.position);
  ...
  tick(dt) {
    ...
    if (this._isPlaying) {
      ...
      // simulation, update particles.
      this._renderer._updateParticles(scaledDeltaTime);

      // update render data
      this._renderer._updateRenderData();
    }
  }
