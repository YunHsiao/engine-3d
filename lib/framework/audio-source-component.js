...
export default class AudioSourceComponent extends Component {
  ...
  set currentTime(num) {
    ...
    if (!this._clip) return;
    this._clip.setCurrentTime(this._currentTime);
  }
...
AudioSourceComponent.schema = {
  clip: {
    ...
    set(val) {
      this._clip = val;
      this._clip.setEntity(this._entity);
      this._clip.setCurrentTime(this._currentTime);
    },
  },
  loop: {
  ...
  volume: {
    ...
    get() { // on some platform volume control may not be available
      if (this._clip) return this._clip.getVolume();
      return this._volume;
    }
  },
};
