...
export default class CannonBody extends CANNON.Body {
  ...
  constructor(options) {
    ...
    this._pull = () => {
      if (!this._entity._hasChanged) return;
   ...
   */
  setUseGravity(val)  {
    if (val) this._world.removeEventListener('beforeStep', this._cancelOutGravity);
    else this._world.addEventListener('beforeStep', this._cancelOutGravity);
  }
  ...
  _setDataflowPushing() {
    ...
    this._world.removeEventListener('beforeStep', this._pull);
  }
  ...
  _setDataflowPulling() {
    ...
    this._world.addEventListener('beforeStep', this._pull);
    this._pull();
  }
