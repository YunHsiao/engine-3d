...
export class Blender2D {
    ...
    _sdGetClosestAngleSamples(pQueried) {
        ...
        for (let i = 0; i < this._samples.length; ++i) {
            ...
            if (vec2.equals(samplePoint, vec2.create(0, 0))) {
                center = i;
                continue;
            }
    ...
    get2DFreeformDirectional(result, pQueried) {
        let axis = vec3.create(0, 0, 0); // buffer for axis
        let tmpV3 = vec3.create(0, 0, 0); // buffer for temp vec3
        let pQueriedProjected = vec3.create(0, 0, 0); // buffer for pQueriedProjected
        let pi3 = vec3.create(0, 0, 0); // buffer for pi3
        let pj3 = vec3.create(0, 0, 0); // buffer for pj3
        let pQueried3 = vec3.create(0, 0, 0); // buffer for pQueried3
        return this._genFreeform(result, (pi, pj, pip, pipj) => {
            let aIJ = 0.0;
            let aIQ = 0.0;
            let angleMultiplier = 2.0;
            vec3.set(pQueriedProjected, pQueried.x, pQueried.y, 0.0);
            if (vec2.equals(pi, vec2.create(0, 0))) {
                aIJ = vec2.angle(pQueried, pj);
                aIQ = 0.0;
                angleMultiplier = 1.0;
            }
            else if (vec2.equals(pj, vec2.create(0, 0))) {
                ...
                if (aIJ <= 0.0)
                    aIQ = 0.0;
                else if (vec2.equals(pQueried, vec2.create(0, 0)))
                    aIQ = aIJ;
                else {
    ...
    _genFreeform(result, influenceFunc) {
        let hiList = [];
        let hiSum = 0;
        let pip = vec2.create(0, 0);
        let pipj = vec2.create(0, 0);
