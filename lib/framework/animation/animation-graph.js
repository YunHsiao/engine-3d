...
export class Motion {
  ...
  set animation(value) {
    this._animation = value ? new ClipOrBlendtree(value) : null;
  }
  ...
  emitClips(result) {
    if (this._animation)
      this._animation.emitClips(result);
  }
  ...
  update(deltaTimeSecs) {
    ...
    { // update blenditems
      ...
      for (let i = 0; i < motions.length; ++i) {
        ...
        motion.emitClips(this._blendItems);
...
export class AnimationGraph {
  /**
   * Construct a new animation graph.
   * @param {string} name Name of the main subgraph/
   */
  constructor(name = "Main") {
     ...
     * @ignore
     */
    this._mainSubgrah = new Subgraph(name);

    /**
     * @type {Subgraph[]}
     */
    this._subgraphs = [ this._mainSubgrah ];
   ...
   */
  get mainSubgraph() {
    return this._mainSubgrah;
  }

  /**
   * Add a new subgraph into this graph.
   * @param {string} name Name of the new subgraph.
   * @return {Subgraph}
   */
  createSubgraph(name) {
    let result = new Subgraph(name);
    this._subgraphs.push(result);
    return result;
  }
