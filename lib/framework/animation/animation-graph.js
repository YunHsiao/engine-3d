...
class _SwitchTaskItem {
   ...
   */
  constructor(clip = null, weight = 0, motion = null) {
    this.set(clip, weight, motion);
  }
   ...
   * @param {Motion} motion Motion related to this clip.
   */
  set(clip, weight, motion) {
    /**
     * @type {Motion} The animation clip.
     */
    this.motion = motion;
    ...
    /**
     * @type {AnimationClip} Motion related to this clip.
     */
    this.clip = clip;
  }
   ...
   */
  getTrueTime() {
    let t = this.motion.time;
    let length = this.clip.length;

    if (this.motion.wrapMode === 'once') {
      if (t > length) {
        t = length;
      }
    } else if (this.motion.wrapMode === 'loop') {
      t %= length;
    } else if (this.motion.wrapMode === 'ping-pong') {
...
class _SwitchTask {
  ...
  constructor() {
    ...
    this._blendItems = new OptimizedValueArray(
      () => new _SwitchTaskItem(),
      (switchTaskItem) => switchTaskItem.set(null, 0, null)
    );
  ...
  update(deltaTimeSecs) {
    ...
    { // update blenditems
      ...
      for (let i = 0; i < motions.length; ++i) {
        ...
        if (motion.animation instanceof AnimationClip)
          this._blendItems.push().set(motion.animation, 1.0, motion);

        else if (motion.animation instanceof BlendTree) {
          let blendedClips = motion.animation.result;
          blendedClips.weightedAnimationClips.forEach((weightedAnimationClip) =>
            this._blendItems.push().set(weightedAnimationClip.animation, weightedAnimationClip.weight, motion));
        }

        if (i == 0 && motions.length == 2)
          this._magic = this._blendItems.size;
      }
    }
    ...
    if (this._timePastSecs >= this._cost) {
      this._blendItems.splice(0, this._magic);
      this._magic = 0;
      this._coff2 = 1.0;
      return;
    }
    ...
    this._coff2 = this._timePastSecs / this._cost;
  ...
  _reset(source, target, cost) {
    ...
    this._coff2 = 0.0;
    this._source = source;
    this._sourceIsBlendtree = this._source instanceof BlendTree;
    this._target = target;
    this._targetIsBlendtree = this._target instanceof BlendTree;
    if (this._magic == 0) {
      this._coff1 = 0.0;
      this._coff2 = 1.0;
    }
   ...
   */
  getWeightCoff(i) {
    return i < this._magic ? this._coff1 : this._coff2;
  }
...
export class AnimationGraph {
  ...
  constructor() {
     ...
     * @ignore
     */
    this._entryMotion = new Motion("__cocos_entry_motion__");
  ...
  update(deltaTimeSecs) {
    ...
    this._curNode.outgoings.forEach((outgoingTransition) => {
      if (!finished && outgoingTransition.conditionsAreSatisfied()) {
        finished = true;
   ...
   * @param {String} wrapMode The animation clip's wrap node.
   * @return {Motion} The newly created motion.
   */
  play(clip, switchCost, speed, wrapMode) {
    let playNode = new Motion(`__auto_generated_motion_${clip.name}__`);
