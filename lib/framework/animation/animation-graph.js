...
export class Motion {
  ...
  constructor(name, animation = null) {
    ...
    /**
     * @type {?(ClipOrBlendtree, MaskedAnimation)}
     */
   ...
   * @type {?(BlendTreeEvalator, AnimationClip, MaskedAnimation)}
   */
  get animation() {
    if (this._animation instanceof ClipOrBlendtree)
  ...
  set animation(value) {
    if ((value instanceof AnimationClip) || (value instanceof BlendTree))
      this._animation = new ClipOrBlendtree(value);
    else if (value instanceof MaskedAnimation)
...
class _SwitchTaskItem {
   ...
   */
  constructor(clip = null, weight = 0, mask = null) {
    this.set(clip, weight, mask);
  }
  ...
  set(clip, weight, mask) {
    ...
    this.weight = weight;
    /**
     * @type {SkeletonMask} The animation clip's mask.
     */
    this.mask = mask;
  }
}
