...
class sphere {
  ...
  /**
   * Set the transform information of the sphere
   *
   * @param {vec3} pos the new position
   * @param {vec3} rot the new rotation
   * @param {vec3} scale the new scale
   * @param {sphere} parent parent sphere to apply
   * @return {sphere}
   */
  setTransform(pos, rot, scale, parent = null) {
    vec3.copy(this.c, pos);
    this.r = maxComponent(scale);
    if (parent) {
      vec3.add(this.c, this.c, parent.c);
      this.r *= parent.r;
    }
    return this;
  }
