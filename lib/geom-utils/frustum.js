...
export default class frustum {
  constructor() {
    ...
    this.planes = new Array(6); // 0: left, 1: right, 2: bottom, 3: top, 4: near, 5: far
    for (let i = 0; i < 6; ++i) {
  ...
  update(m, inv) {
    ...
    vec3.set(_v3,  1,  1,  1); vec3.transformMat4(this.vertices[0], _v3, inv);
    vec3.set(_v3, -1,  1,  1); vec3.transformMat4(this.vertices[1], _v3, inv);
    vec3.set(_v3, -1, -1,  1); vec3.transformMat4(this.vertices[2], _v3, inv);
    vec3.set(_v3,  1, -1,  1); vec3.transformMat4(this.vertices[3], _v3, inv);
    vec3.set(_v3,  1,  1, -1); vec3.transformMat4(this.vertices[4], _v3, inv);
    vec3.set(_v3, -1,  1, -1); vec3.transformMat4(this.vertices[5], _v3, inv);
    vec3.set(_v3, -1, -1, -1); vec3.transformMat4(this.vertices[6], _v3, inv);
    vec3.set(_v3,  1, -1, -1); vec3.transformMat4(this.vertices[7], _v3, inv);
  }
}
