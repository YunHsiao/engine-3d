 ...
 * @param {aabb} aabb
 * @param {plane} plane
 * @return {number} outside(front) = 0, inside(back) = -1, intersect = 1
 */
...
let sphere_frustum_accurate = (function () {
  ...
  return function (sphere, frustum) {
    for (let i = 0; i < 6; i++) {
      ...
      // frustum plane normal points to the inside
      if (dot + r < d) return false; // completely outside
      ...
      for (let j = 0; j < 6; j++) {
        ...
        if (vec3.dot(test.n, pt) < test.d) return false;
      }
    }
    return true;
  };
