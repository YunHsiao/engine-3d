...
class quat {
  ...
  static toEuler(out, q) {
    ...
    let test = y * w - x * z;
    if (test > 0.5 - Math.Epsilon) { // singularity at north pole
      heading = 2 * Math.atan2(x, w);
      attitude = Math.PI / 2;
      bank = 0;
    }
    if (test < Math.Epsilon - 0.5) { // singularity at south pole
      heading = -2 * Math.atan2(x, w);
      attitude = -Math.PI / 2;
      bank = 0;
    }
    if(isNaN(heading)){
      ...
      let z2 = z*z;
      let w2 = w*w;
      heading = Math.atan2(2*(x*y+z*w), w2+x2-y2-z2); // heading
      attitude = Math.asin(2*test); // attitude
      bank = Math.atan2(2*(x*w+y*z), w2-x2-y2+z2); // bank
    }

    out.z = toDegree(heading);
    out.y = toDegree(attitude);
    out.x = toDegree(bank);
