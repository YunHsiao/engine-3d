...
class quat {
  ...
  static toEuler(out, q) {
    ...
    let test = x * y + z * w, yaw, pitch, roll;
    ...
    if(isNaN(yaw)){
      ...
      let sqy = y * y;
      let sqz = z * z;
      yaw = Math.atan2(2*y*w - 2*x*z , 1 - 2*sqy - 2*sqz); // yaw
      pitch = Math.asin(2 * test); // pitch
      roll = Math.atan2(2*x*w - 2*y*z , 1 - 2*sqx - 2*sqz); // roll
    }

    out.y = toDegree(yaw);
    out.z = toDegree(pitch);
    out.x = toDegree(roll);
