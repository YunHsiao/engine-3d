...
export default class Device {
  ...
  _initExtensions(extensions) {
    ...
    for (let i = 0; i < extensions.length; ++i) {
      ...
      try {
        ...
        if (ext) {
          console.log(`Existension ${name} is supported.`);
          this._extensions[name] = ext;
        }
        else {
          console.log(`Existension ${name} is NOT supported.`);
        }
      } catch (e) {
        console.error(e);
      }
  ...
  draw(base, count) {
    ...
    for (let i = 0; i < next.program._uniforms.length; ++i) {
      ...
      let isArray = Array.isArray(uniform.value);
      if (uniformInfo.size !== undefined && !isArray) {
        console.warn(`Uniform ${uniformInfo.name} needs array argument.`);
        continue;
      }

      if (isArray && (uniform.value.length > uniformInfo.size)) {
        console.warn(`Elements count of input argument for uniform ${uniformInfo.name} is out of bounds.`);
      }
      ...
      if (!isArray) {
        ...
        //debugger;
        let n = Math.min(uniform.value.length, uniformInfo.size);
        for (let i = 0; i < n; ++i) {
          commitFunc(gl, uniformInfo.elementLocations[i], uniform.value[i]);
        }
      }
    }
